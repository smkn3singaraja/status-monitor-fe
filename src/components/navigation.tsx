...
import { getLatestStatusClient, getHistoricalStatusClient } from '@/lib/api-client';
...
export function Navigation() {
    ...
    const prefetchData = (href: string) => {
        if (href === '/') {
            getLatestStatusClient();
        } else if (href === '/historical') {
            const end = new Date();
        ...
        <nav className="border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex h-16 items-center justify-between">
                    <Link href="/" className="flex items-center gap-3 group" onMouseEnter={() => prefetchData('/')}>
